...
def boxes3d_lidar_to_aligned_bev_boxes_v1(boxes3d):
    """
    Args:
        boxes3d: (N, 7 + C) [x, y, z, dx, dy, dz, heading] in lidar coordinate
    ...
    Returns:
        aligned_bev_boxes: (N, 4) [x1, y1, x2, y2] in the above lidar coordinate
    """
    dx = abs(torch.cos(boxes3d[:, -1]) * boxes3d[:, 3]) + abs(torch.sin(boxes3d[:, -1]) * boxes3d[:, 4])
    dy = abs(torch.sin(boxes3d[:, -1]) * boxes3d[:, 3]) + abs(torch.cos(boxes3d[:, -1]) * boxes3d[:, 4])
    x1 = boxes3d[:, 0] - dx / 2
    x2 = boxes3d[:, 0] + dx / 2
    y1 = boxes3d[:, 1] - dy / 2
    y2 = boxes3d[:, 1] + dy / 2
    aligned_bev_boxes = torch.cat((x1.unsqueeze(-1), y1.unsqueeze(-1), x2.unsqueeze(-1), y2.unsqueeze(-1)), -1)
    return aligned_bev_boxes
...
def boxes3d_nearest_bev_iou_v1(boxes_a, boxes_b):
    """
    Args:
        boxes_a: (N, 7) [x, y, z, dx, dy, dz, heading]
        boxes_b: (N, 7) [x, y, z, dx, dy, dz, heading]
    ...
    """
    boxes_bev_a = boxes3d_lidar_to_aligned_bev_boxes_v1(boxes_a)
    boxes_bev_b = boxes3d_lidar_to_aligned_bev_boxes_v1(boxes_b)
