...
class VoxelResBackBone8xMyNet(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        input_sp_tensor = spconv.SparseConvTensor(
            ...
            spatial_shape=self.sparse_shape,  # array([  41, 1472, 1552])
...
# xnd
class PillarBackBone8x_v1(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
        ...
        # conv0
        num = '0'
        self.conv110 = spconv.SparseSequential(
            block(channels[0], channels[0], spconv_kernel_sizes[0], norm_fn=norm_fn, stride=2, padding=int(spconv_kernel_sizes[0] // 2), indice_key='spconv'+num, conv_type='spconv'),
            SparseBasicBlock(channels[0], channels[0], norm_fn=norm_fn, indice_key='res'+num),
        )
        self.conv110_2 = SPConvFuse(channels[0], channels[0], norm_fn=norm_fn, indice_key='fuse12')

        # conv2
        num = '2'
        self.conv112 = spconv.SparseSequential(
            block(channels[0], channels[1], spconv_kernel_sizes[1], norm_fn=norm_fn, stride=2, padding=int(spconv_kernel_sizes[1]//2), indice_key='spconv'+num, conv_type='spconv'),
            SparseBasicBlock(channels[1], channels[1], norm_fn=norm_fn, indice_key='res'+num),
        )
        self.conv112_2 = SparseConvBasicBlock(channels[1], channels[1], stride=1, norm_fn=norm_fn, indice_key='subm'+num+'2', sparse_shape=self.sparse_shape)
        # conv3
        num = '3'
        self.conv113 = spconv.SparseSequential(
            block(channels[1], channels[2], spconv_kernel_sizes[2], norm_fn=norm_fn, stride=2, padding=int(spconv_kernel_sizes[2]//2), indice_key='spconv'+num, conv_type='spconv'),
            SparseBasicBlock(channels[2], channels[2], norm_fn=norm_fn, indice_key='res'+num),
        )
        self.conv113_2 = SparseConvBasicBlock(channels[2], channels[2], stride=1, norm_fn=norm_fn, indice_key='subm'+num+'2', sparse_shape=self.sparse_shape)
        # conv4
        num = '4'
        self.conv114 = spconv.SparseSequential(
            block(channels[2], channels[3], spconv_kernel_sizes[3], norm_fn=norm_fn, stride=2, padding=int(spconv_kernel_sizes[3]//2), indice_key='spconv'+num, conv_type='spconv'),
            SparseBasicBlock(channels[3], channels[3], norm_fn=norm_fn, indice_key='res'+num),
        )
        self.conv114_2 = SparseConvBasicBlock(channels[3], channels[3], stride=1, norm_fn=norm_fn, indice_key='subm'+num+'2', sparse_shape=self.sparse_shape)
    ...
    def forward(self, batch_dict):
        ...
        x_conv0 = self.conv110(x)
        x_conv0_2 = self.conv110_2(x, x_conv0)

        x_conv2 = self.conv112(x_conv0_2)
        x_conv2_2 = self.conv112_2(x_conv2)
        
        x_conv3 = self.conv113(x_conv2_2)
        x_conv3_2 = self.conv113_2(x_conv3)

        x_conv4 = self.conv114(x_conv3_2)
        x_conv4_2 = self.conv114_2(x_conv4)

        out = self.bev_out(x_conv4_2)
...
# 0903 xnd1
class PillarBackBone8x_v0903(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
        ...
        SparseConvBasicBlock = SparseConvBasicBlock_1
...
# epoch500_0830_1 xnd
class PillarBackBone8x_v08301(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
...
# 0815_2
class PillarBackBone8x_v0815_2(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
        ...
        self.conv1_2 = spconv.SparseSequential(
            # block(channels[0], channels[0], spconv_kernel_sizes[0], norm_fn=norm_fn, stride=2, padding=int(spconv_kernel_sizes[0]//2), indice_key='spconv12', conv_type='spconv'),
            # block(channels[0], channels[0], spconv_kernel_sizes[0], norm_fn=norm_fn, padding=1, indice_key='subm1'),
            # block(channels[0], channels[0], spconv_kernel_sizes[0], norm_fn=norm_fn, padding=1, indice_key='subm1'),
            SparseBasicBlock(channels[0], channels[0], norm_fn=norm_fn, indice_key='res12'),
            SparseBasicBlock(channels[0], channels[0], norm_fn=norm_fn, indice_key='res12'),
        )
        ...
        self.conv2_2 = spconv.SparseSequential(
            ...
            # block(channels[1], channels[1], spconv_kernel_sizes[1], norm_fn=norm_fn, padding=1, indice_key='subm2'),
            # block(channels[1], channels[1], spconv_kernel_sizes[1], norm_fn=norm_fn, padding=1, indice_key='subm2'),
        ...
        self.conv3_2 = spconv.SparseSequential(
            ...
            # block(channels[2], channels[2], spconv_kernel_sizes[2], norm_fn=norm_fn, padding=1, indice_key='subm3'),
            # block(channels[2], channels[2], spconv_kernel_sizes[2], norm_fn=norm_fn, padding=1, indice_key='subm3'),
        ...
        self.conv4_2 = spconv.SparseSequential(
            ...
            # block(channels[3], channels[3], spconv_kernel_sizes[3], norm_fn=norm_fn, padding=1, indice_key='subm4'),
            # block(channels[3], channels[3], spconv_kernel_sizes[3], norm_fn=norm_fn, padding=1, indice_key='subm4'),
...
# 0815_1
class PillarBackBone8x_v0815_1(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
...
# 0815
class PillarBackBone8x_v0815(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
...
# 0814_1
class PillarBackBone8x_v0814(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
...
# epoch500_0809_2
class PillarBackBone8x_v0809_2(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
...
# epoch500_0809
class PillarBackBone8x_v0809(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
...
# epoch500_0809_2d
class PillarBackBone8x_v0809_2d(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
...
# epoch500_0809 ablation study
class PillarBackBone8x_v1_as(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
...
# 虚拟点 xnd
class PillarBackBone8x_vxnd1(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
        ...
        # conv1
        num = '1'
        self.conv111 = spconv.SparseSequential(
            block(channels[0], channels[0], spconv_kernel_sizes[0], norm_fn=norm_fn, stride=2, padding=int(spconv_kernel_sizes[0]//2), indice_key='spconv'+num, conv_type='spconv'),
            # block(channels[0], channels[0], spconv_kernel_sizes[0], norm_fn=norm_fn, padding=1, indice_key='subm1'),
            SparseBasicBlock(channels[0], channels[0], norm_fn=norm_fn, indice_key='res'+num),
            # SparseBasicBlock(channels[0], channels[0], norm_fn=norm_fn, indice_key='res1'),
        )
        self.conv111_2 = SparseConvBasicBlock(channels[0], channels[0], stride=1, norm_fn=norm_fn, indice_key='subm'+num+'2', sparse_shape=self.sparse_shape)
        # conv2
        num = '2'
        self.conv112 = spconv.SparseSequential(
            block(channels[0], channels[1], spconv_kernel_sizes[1], norm_fn=norm_fn, stride=2, padding=int(spconv_kernel_sizes[1]//2), indice_key='spconv'+num, conv_type='spconv'),
    ...
    def forward(self, batch_dict):
        ...
        x_conv1 = self.conv111(x)
        x_conv1_2 = self.conv111_2(x_conv1)

        x_conv2 = self.conv112(x_conv1_2)
        x_conv2_2 = self.conv112_2(x_conv2)
