...
class Detector3DTemplate(nn.Module):
    ...
    def load_params_from_file(self, filename, logger, to_cpu=False, pre_trained_path=None, gpu_number=None):
        if not os.path.isfile(filename):
            raise FileNotFoundError

        logger.info('==> Loading parameters from checkpoint %s to %s' % (filename, 'CPU' if to_cpu else 'GPU'))
        loc_type = torch.device('cpu') if to_cpu else None
        map_loc = 'cuda:%s' % str(gpu_number) if gpu_number is not None else None
        checkpoint = torch.load(filename, map_location=map_loc if loc_type is None else loc_type)
