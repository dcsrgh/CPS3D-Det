...
# ATE
def calculate_3d_ate(gt_boxes: np.ndarray, dt_boxes: np.ndarray):
    """
    L2 distance between the box centers (xy only).
    :param gt_boxes: GT annotation sample.
    :param dt_boxes: Predicted sample.
    :return: L2 distance.
    """
    N, K = gt_boxes.shape[0], dt_boxes.shape[0]
    ates = np.zeros((N, K), dtype=gt_boxes.dtype)
    for i in range(N):
        for j in range(K):
            value = np.linalg.norm(dt_boxes[j] - gt_boxes[i])
            ates[i][j] = value
    return ates


# ATE_v2
def calculate_3d_ate_v2(gt_boxes: np.ndarray, dt_boxes: np.ndarray):
    """
    L2 distance between the box centers (xy only).
    :param gt_boxes: GT annotation sample.
    :param dt_boxes: Predicted sample.
    ...
    ates = []
    for i in range(N):
        value = np.linalg.norm(dt_boxes - gt_boxes[i])
        ates.append(value / len(dt_boxes))
    return np.array(ates, dtype=gt_boxes.dtype)


# ASE
def calculate_3d_ase(gt_boxes: np.ndarray, dt_boxes: np.ndarray):
    """
    ...
    """
    # Validate inputs.
    sa_size = np.array(gt_boxes)
    sr_size = np.array(dt_boxes)
    # assert all(sa_size > 0), 'Error: gt_boxes sizes must be >0.'
    # assert all(sr_size > 0), 'Error: dt_boxes sizes must be >0.'
    assert np.sum(sa_size < 0) == 0
    assert np.sum(sr_size < 0) == 0

    N, K = sa_size.shape[0], sr_size.shape[0]
    ious = np.zeros((N, K), dtype=sa_size.dtype)
    for i in range(N):
        for j in range(K):
            # Compute IOU.
            min_wlh = np.minimum(sa_size[i], sr_size[j])
            volume_sa = np.prod(sa_size[i])
            volume_sr = np.prod(sr_size[j])
            volume_wlh = np.prod(min_wlh)
            union = volume_sa + volume_sr - volume_wlh
            iou = volume_wlh / union
            ious[i][j] = 1 - iou
    return ious
...
# AOE
def calculate_3d_aoe(gt_box: np.ndarray, eval_box: np.ndarray, period: float = np.pi):
    """
    ...
    N, K = gt_box.shape[0], eval_box.shape[0]
    aoes = np.zeros((N, K), dtype=gt_box.dtype)
    period = np.pi
    for i in range(N):
        yaw_gt = quaternion_yaw(gt_box[i])
        for j in range(K):
            yaw_est = quaternion_yaw(eval_box[j])
            aoes[i][j] = abs(angle_diff(yaw_gt, yaw_est, period))
    return aoes


# AVE
def calculate_3d_ave(gt_box: np.ndarray, pred_box: np.ndarray):
    """
...
# AAE
def calculate_3d_aae(gt_box: np.ndarray, pred_box: np.ndarray):
    """
...
# d3_box_overlap(gt_boxes, dt_boxes, rinc, z_axis=z_axis, z_center=z_center).astype(np.float64)
def d3_box_overlap_v1(boxes, qboxes, rinc, criterion=-1, z_axis=1, z_center=1.0):
    """kitti camera format z_axis=1.
    """
    d3_box_overlap_kernel(boxes, qboxes, rinc, criterion, z_axis, z_center)
    return rinc
...
def calculate_iou_partly_1(gt_annos,
                         dt_annos,
                         metric,
                         num_parts=50,
                         z_axis=1,
    ...
    for num_part in split_parts:
        ...
        if metric == 0:  # anno['bbox']
            ...
            dt_boxes = np.concatenate([a["bbox"] for a in dt_annos_part], 0)
            overlap_part = image_box_overlap(gt_boxes, dt_boxes)
        elif metric == 1:  # anno['bev']
            gt_boxes = np.concatenate([a["bev"] for a in gt_annos_part], 0)
            dt_boxes = np.concatenate([a["bev"] for a in dt_annos_part], 0)
            overlap_part = image_box_overlap(gt_boxes, dt_boxes)
        elif metric == 2:  # 3D
            gt_bevs = np.concatenate([a["bev"] for a in gt_annos_part], 0)
            dt_bevs = np.concatenate([a["bev"] for a in dt_annos_part], 0)
            rinc = image_box_overlap(gt_bevs, dt_bevs, criterion=2)
            ...
            overlap_part = d3_box_overlap_v1(gt_boxes, dt_boxes, rinc, z_axis=z_axis, z_center=z_center).astype(np.float64)
        elif metric in [3, 4, 5]:  # NDS  3:ate  4:ase  5:aoe
            class_name = 'Car'
            gt_bevs = np.concatenate([a["bev"][a['name'] == class_name] for a in gt_annos_part], 0)
            dt_bevs = np.concatenate([a["bev"][a['name'] == class_name] for a in dt_annos_part], 0)
            rinc = image_box_overlap(gt_bevs, dt_bevs, criterion=2)
            
            loc = np.concatenate([a["location"][a['name'] == class_name] for a in gt_annos_part], 0)
            dims = np.concatenate([a["dimensions"][a['name'] == class_name] for a in gt_annos_part], 0)
            rots = np.concatenate([a["rotation_y"][a['name'] == class_name] for a in gt_annos_part], 0)
            gt_boxes = np.concatenate([loc, dims, rots[..., np.newaxis]], axis=1)

            loc = np.concatenate([a["location"][a['name'] == class_name] for a in dt_annos_part], 0)
            dims = np.concatenate([a["dimensions"][a['name'] == class_name] for a in dt_annos_part], 0)
            rots = np.concatenate([a["rotation_y"][a['name'] == class_name] for a in dt_annos_part], 0)
            dt_boxes = np.concatenate([loc, dims, rots[..., np.newaxis]], axis=1)

            if metric == 3:
                overlap_part = calculate_3d_ate(gt_boxes[:, :2], dt_boxes[:, :2])
                ate1 = np.sum(overlap_part) / (len(dt_boxes)) / (len(gt_boxes))
                ate1s.append(ate1)

                overlap_part_v2 = calculate_3d_ate_v2(gt_boxes[:, :2], dt_boxes[:, :2])
                ate2 = np.sum(overlap_part_v2) / len(overlap_part_v2)
                ate2s.append(ate2)
            if metric == 4:
                overlap_part = calculate_3d_ase(gt_boxes[:, 3:6], dt_boxes[:, 3:6])
                ases.append(np.sum(overlap_part) / (len(gt_boxes) + len(dt_boxes)))
            if metric == 5:
                overlap_part = calculate_3d_aoe(gt_boxes[:, -1], dt_boxes[:, -1])
        
            if isinstance(overlap_part, np.ndarray):
                    pass
            else:
                overlap_part = np.array(overlap_part, dtype=gt_boxes.dtype)
        # elif metric in [3, 4, 5]:  # NDS  3:ate  4:ase  5:aoe
        #     class_name = 'Car'
        #     gt_bevs = np.concatenate([a["bev"][a['name'] == class_name] for a in gt_annos_part], 0)
        #     dt_bevs = np.concatenate([a["bev"][a['name'] == class_name] for a in dt_annos_part], 0)
    ...
    example_idx = 0
    if len(ate1s) > 0:
        print('ate1s:--------------------------------' + str(np.sum(ate1s) / len(ate1s)))
    if len(ate2s) > 0:
        print('ate2s:--------------------------------' + str(np.sum(ate2s) / len(ate2s)))
    if len(ases) > 0:
        print('ases:--------------------------------' + str(np.sum(ases) / len(ases)))
    for j, num_part in enumerate(split_parts):
        gt_annos_part = gt_annos[example_idx:example_idx + num_part]
        dt_annos_part = dt_annos[example_idx:example_idx + num_part]
        gt_num_idx, dt_num_idx = 0, 0
...
def eval_class(gt_annos,
    ...
    for m, current_class in enumerate(current_classes):
        for l, difficulty in enumerate(difficultys):
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric % 3, m]):
                thresholdss = []
                for i in range(len(gt_annos)):
                ...
                for i in range(len(thresholds)):
                    precision[m, l, k, i] = 0 if (pr[i, 0] + pr[i, 1]) == 0 else pr[i, 0] / (pr[i, 0] + pr[i, 1])
                    if compute_aos:
                        aos[m, l, k, i] = 0 if (pr[i, 0] + pr[i, 1]) == 0 else pr[i, 3] / (pr[i, 0] + pr[i, 1])
                for i in range(len(thresholds)):
...
def get_mAP(prec):
    ...
    # for i in range(1, prec.shape[-1], 4):
        sums = sums + prec[..., i]
    return sums / 11 * 100
...
def do_eval_v4(gt_annos,
    ...
    mAP_bbox_R40 = get_mAP_v2(ret["precision"])
    mAP_aos = None
    mAP_aos_R40 = None
    if compute_aos:
        mAP_aos = get_mAP(ret["orientation"])
        mAP_aos_R40 = get_mAP_v2(ret["orientation"])
    ret = eval_class(
        gt_annos,
        dt_annos,
        current_classes,
    ...
    mAP_bev_R40 = get_mAP_v2(ret["precision"])
    ret = eval_class(
        gt_annos,
        dt_annos,
        current_classes,
...
def do_eval_nds(gt_annos,
    ...
    ret = eval_class(
        ...
        z_axis=z_axis,
        z_center=z_center)
    ate_3d = get_mAP(ret["precision"])
    ate_3d_R40 = get_mAP_v2(ret["precision"])
    ...
    ret = eval_class(
        ...
        z_axis=z_axis,
        z_center=z_center)
    ase_3d = get_mAP(ret["precision"])
    ase_3d_R40 = get_mAP_v2(ret["precision"])
    ...
    #     z_axis=z_axis,
    #     z_center=z_center)
    # aoe_3d = get_mAP(ret["precision"])
    # aoe_3d_R40 = get_mAP_v2(ret["precision"])

    return ate_3d, ase_3d, ate_3d_R40, ase_3d_R40
...
def get_official_eval_result(gt_annos,
    ...
    compute_nds = False
    if compute_nds:
        ate_3d, ase_3d, ate_3d_R40, ase_3d_R40 = do_eval_nds(
            gt_annos, dt_annos, current_classes, min_overlaps, compute_aos, difficultys, z_axis=z_axis, z_center=z_center)

    mAPbbox, mAPbev, mAP3d, mAPaos, mAPbbox_R40, mAPbev_R40, mAP3d_R40, mAPaos_R40 = do_eval_v4(
        gt_annos, dt_annos, current_classes, min_overlaps, compute_aos, difficultys, z_axis=z_axis, z_center=z_center)
    ...
    for j, curcls in enumerate(current_classes):
        ...
        for i in range(min_overlaps.shape[0]):
            ...
            if compute_nds:
                result += print_str((f"ate  AP:{ate_3d[j, 0, i]:.2f}, "
                                     f"{ate_3d[j, 1, i]:.2f}, "
                                     f"{ate_3d[j, 2, i]:.2f}"))
                result += print_str((f"ase  AP:{ase_3d[j, 0, i]:.2f}, "
                                     f"{ase_3d[j, 1, i]:.2f}, "
                                     f"{ase_3d[j, 2, i]:.2f}"))
                # result += print_str((f"aoe  AP:{aoe_3d[j, 0, i]:.2f}, "
                #                      f"{aoe_3d[j, 1, i]:.2f}, "
                #                      f"{aoe_3d[j, 2, i]:.2f}"))
            ...
            if compute_nds:
                result += print_str((f"ate  AP:{ate_3d_R40[j, 0, i]:.2f}, "
                                     f"{ate_3d_R40[j, 1, i]:.2f}, "
                                     f"{ate_3d_R40[j, 2, i]:.2f}"))
                result += print_str((f"ase  AP:{ase_3d_R40[j, 0, i]:.2f}, "
                                     f"{ase_3d_R40[j, 1, i]:.2f}, "
                                     f"{ase_3d_R40[j, 2, i]:.2f}"))
                # result += print_str((f"aoe  AP:{aoe_3d_R40[j, 0, i]:.2f}, "
                #                      f"{aoe_3d_R40[j, 1, i]:.2f}, "
                #                      f"{aoe_3d_R40[j, 2, i]:.2f}"))
