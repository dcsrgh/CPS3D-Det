...
class CustomDataset(DatasetTemplate):
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        ...
        def kitti_eval(eval_det_annos, eval_gt_annos, map_name_to_kitti):
            from ..kitti.kitti_object_eval_python import eval_1 as kitti_eval
            from ..kitti import kitti_utils

            eval_det_annos = kitti_utils.transform_annotations_to_kitti_format_v1(annos=eval_det_annos, map_name_to_kitti=map_name_to_kitti, is_gt=False)
            eval_gt_annos = kitti_utils.transform_annotations_to_kitti_format_v1(
                annos=eval_gt_annos, map_name_to_kitti=map_name_to_kitti,
                info_with_fakelidar=self.dataset_cfg.get('INFO_WITH_FAKELIDAR', False), is_gt=True
            )
    ...
    def get_custom_obj_level(self, gt_points_num):
        if 400 < gt_points_num:
            self.level_str = 'Easy'
            return 0  # Easy
        elif 150 < gt_points_num <= 400:
            self.level_str = 'Moderate'
            return 1  # Moderate
        elif 0 < gt_points_num <= 150:
            self.level_str = 'Hard'
            return 2  # Hard
        else:
    ...
    def create_groundtruth_database(self, info_path=None, used_classes=None, split='train'):
        ...
        for k in range(len(infos)):
            ...
            for i in range(num_obj):
                ...
                level = self.get_custom_obj_level(gt_points.shape[0])
...
def create_custom_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    ...
    train_split, val_split, normal_split = 'train', 'val', 'normal'
    ...
    normal_filename = save_path / ('custom_infos_%s.pkl' % normal_split)
    ...
    dataset.set_split(normal_split)
    custom_infos_normal = dataset.get_infos(
        class_names, num_workers=workers, has_label=False, num_features=num_features
    )
    with open(normal_filename, 'wb') as f:
        pickle.dump(custom_infos_normal, f)
    print('Custom info train file is saved to %s' % normal_filename)
